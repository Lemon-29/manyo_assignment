...
describe 'タスクモデル機能', type: :model do
  describe 'バリデーションのテスト' do
    context 'タスクのタイトルが空の場合' do
      it 'バリデーションにひっかる' do
        task = Task.new(title: '', content: '失敗テスト', expired_at: '2021-03-33 03:33:33', status:1)
    ...
    context 'タスクの詳細が空の場合' do
      it 'バリデーションにひっかかる' do
        task = Task.new(title: '失敗テスト', content: ''), expired_at: '2021-03-33 03:33:33', status:1
        expect(task).not_to be_valid
      end
    end
  ...
  describe '検索機能' do
    let!(:task) { FactoryBot.create(:task, title: 'task', expired_at: '2021-03-33 03:33:33', status: 1) }
